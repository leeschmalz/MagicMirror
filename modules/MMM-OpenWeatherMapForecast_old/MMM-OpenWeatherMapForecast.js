...
Module.register("MMM-OpenWeatherMapForecast", {
    ...
    */
	...
	start: function () {
		...
		this.sanitizeNumbers(["updateInterval", "requestDelay", "hourlyForecastInterval", "maxHourliesToShow", "maxDailiesToShow", "mainIconSize", "forecastIconSize", "updateFadeSpeed", "animatedIconPlayDelay"]);
    ...
    */
	processWeatherData: function () {
		...
		if (this.config.concise) {
			...
			summary =
				this.weatherData.current.weather[0].description + "." + " " + (this.weatherData.hourly ? this.weatherData.hourly[0].weather[0].description + " " : "") + (this.weatherData.daily ? this.weatherData.daily[0].weather[0].description : "");
		}
    ...
    */
	...
	generateIconSrc: function (icon, mainIcon) {
		if (mainIcon) {
			return this.file("icons/" + this.iconsets[this.config.mainIconset].path + "/" + icon + "." + this.iconsets[this.config.mainIconset].format);
		}
		return this.file("icons/" + this.iconsets[this.config.iconset].path + "/" + icon + "." + this.iconsets[this.config.iconset].format);
	},
