...
Module.register("MMM-OpenWeatherMapForecast", {
  ...
  socketNotificationReceived: function (notification, payload) {
    if (
      notification === "OPENWEATHER_ONE_CALL_FORECAST_DATA" &&
      ...
      this.sendNotification(
        "OPENWEATHER_ONE_CALL_FORECAST_WEATHER_UPDATE",
        payload
      );
  ...
  formatPrecipitation: function (
    ...
    if (snowAccumulation) {
      ...
      if (typeof snowAccumulation === "number") {
        ...
        if (this.getUnit("accumulationSnow") === "in") {
          accumulation =
            Math.round((snowAccumulation["1h"] * 100) / 25.4) / 100;
        } else {
      ...
      if (typeof rainAccumulation === "number") {
        ...
        if (this.getUnit("accumulationRain") === "in") {
          accumulation =
            Math.round((rainAccumulation["1h"] * 100) / 25.4) / 100;
        } else {
  ...
  generateIconSrc: function (icon, mainIcon) {
    if (mainIcon) {
      return this.file(
        "icons/" +
          this.iconsets[this.config.mainIconset].path +
          "/" +
          icon +
          "." +
