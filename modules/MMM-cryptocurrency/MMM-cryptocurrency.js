		...
		for (var j = 0; j < apiResult.length; j++) {
			...
			if (this.imageExists(apiResult[j].slug)) {
				...
				this.sendNotification("SHOW_ALERT", {
					...
					message: "" + this.translate("IMAGE") + " " + apiResult[j].slug + ".png " + this.translate("NOTFOUND") + " /MMM-cryptocurrency/public/" + this.folder
				});
			}
			...
			if (this.config.showGraphs) {
				...
				if (this.sparklineIds[apiResult[j].slug]) {
					...
					graph.src = "https://s3.coinmarketcap.com/generated/sparklines/web/7d/usd/" + this.sparklineIds[apiResult[j].slug] + ".svg?cachePrevention=" + Math.random();
