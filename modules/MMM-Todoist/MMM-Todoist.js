...
Module.register("MMM-Todoist", {
	...
	addTodoTextCell: function (item) {
		...
		return this.createCell("title bright alignLeft", this.shorten(para[0].innerHTML, this.config.maxTitleLength, this.config.wrapEvents));
	...
	getDom: function () {
		...
		if (this.config.fade && this.config.fadePoint < 1)
			divTable
				...
				.forEach((row, i, rows) => (row.style.opacity = Math.max(0, Math.min(1 - (((i + 1) * (1 / rows.length) - this.config.fadePoint) / (1 - this.config.fadePoint)) * (1 - this.config.fadeMinimumOpacity), 1))));
