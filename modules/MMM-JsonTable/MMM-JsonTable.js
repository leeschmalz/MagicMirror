...
Module.register("MMM-JsonTable", {
	...
	// Request node_helper to get json from filepath
	getJson() {
		this.sendSocketNotification("MMM-JsonTable_GET_JSON", this.config.filepath);
	},
	...
	socketNotificationReceived(notification, payload) {
		if (notification === "MMM-JsonTable_JSON_RESULT") {
			...
			if (payload.filepath === this.config.filepath) {
	...
	getTableRow(jsonObject) {
		...
		Object.entries(jsonObject).forEach(([key, value]) => {
			...
			} else if (this.config.keepColumns.length === 0 || this.config.keepColumns.indexOf(key) >= 0) {
